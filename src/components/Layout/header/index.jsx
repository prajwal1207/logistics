    ...
    <header className="w-full border-b shadow-md z-[999] relative">
      <div className="w-full h-auto text-white font-bold bg-gradient-to-r from-white via-[#c32126] to-red-700 bg-[length:200%_200%] animate-subtleGradient">
        <div className="max-w-7xl mx-auto px-4 py-2 flex flex-wrap justify-end items-end gap-2 text-sm sm:text-[14px]">
          <a href="https://wa.me/917000448907" target="_blank" rel="noopener noreferrer" className="flex items-center hover:underline">
            <FaWhatsapp size="1.5rem" />
          </a>
          <a href="tel:+917000448907" className="flex items-center gap-2 hover:underline">
            <FaPhoneAlt size="1.2rem" />
            +91 7000448907
          </a>
          <a href="mailto:info@adhuniktransport.com" className="flex items-center gap-2 hover:underline">
      ...
      <nav className="bg-white w-full py-3 flex justify-between items-center relative px-4 sm:px-8 md:px-16">
            ...
            <div key={index} className="relative">
              {item.dropdown ? (
                <button
                  ...
                  className="flex items-center gap-1 focus:outline-none hover:text-[#c32126] transition"
                >
                ...
                <Link
                  ...
                  className="hover:text-[#c32126] transition"
                >
                  {item.label}
                </Link>
              )}
              ...
              {item.dropdown && activeDropdown === index && (
                <div className="absolute top-full left-0 mt-2 w-64 bg-white border shadow-md z-50">
                  {item.dropdown.map((subItem, subIdx) => (
                    <Link
                      key={subIdx}
                      to={subItem.href}
                      onClick={() => setActiveDropdown(null)}
                      className="block px-4 py-2 text-md text-gray-700 hover:bg-gray-100 hover:text-[#c32126] transition"
                    >
                      {subItem.label}
                    </Link>
                  ))}
        ...
        {/* Mobile Overlay */}
        {mobileMenuOpen && (
          <div onClick={closeMobileMenu} className="fixed inset-0 bg-black opacity-40 z-[30]"></div>
        )}
        ...
        {mobileMenuOpen && (
          <div className="fixed inset-0 top-0 left-0 w-full h-full bg-white z-[40] flex flex-col">
            <div className="flex justify-between items-center p-4 border-b">
              ...
              <button onClick={closeMobileMenu} className="text-gray-800 text-2xl">
                <RxCross2 />
              </button>
            </div>
            ...
            <div className="overflow-y-auto px-4 pt-4 pb-10 flex-1">
              {menuItems.map((item, index) => (
                <div key={index} className="border-b">
                  {item.dropdown ? (
                    <>
                      <div className="py-3 text-gray-800 font-medium">{item.label}</div>
                      <div className="pl-4 pb-2">
                        {item.dropdown.map((subItem, subIdx) => (
                          <Link
                            ...
                            className="block px-4 py-2 text-md text-gray-600 hover:bg-gray-100 hover:text-[#c32126] transition"
                          >
                            {subItem.label}
                          </Link>
                        ))}
                    ...
                    <Link
                      to={item.href}
                      onClick={closeMobileMenu}
                      className="block py-3 text-gray-800 font-medium hover:bg-gray-100 hover:text-[#c32126] transition"
                    >
                      {item.label}
                    </Link>
                  )}
